...
describe('useCanvasContentSize', () => {
  ...
  it('uses the furthest element for bounds', () => {
    const store = useElementsStore()
    store.addShape('rectangle', 0, 0)
    store.addShape('rectangle', 500, 600)

    const { canvasContentSize } = useCanvasContentSize()

    expect(canvasContentSize.value.width).toBe(600 + 50) // 500 + 100 + 50
    expect(canvasContentSize.value.height).toBe(700 + 50) // 600 + 100 + 50
  })
})
